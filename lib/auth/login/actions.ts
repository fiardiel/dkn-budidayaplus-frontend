...
async function setRefreshAndAccessTokens(response: Response) {
  ...
  cookies().set("refreshToken", refresh, { path: "/", httpOnly: true, secure: true, sameSite: "lax" });
  cookies().set("accessToken", access, { path: "/", httpOnly: true, secure: true, sameSite: "lax" });
}
